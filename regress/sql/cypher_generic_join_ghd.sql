...
BEGIN
    ...
    IF NOT has_generic OR NOT has_component_count OR
       ...
       NOT has_separator_domain OR NOT has_core_pruning OR
 ...
 * directory instead of rebuilding raw row ranges for those recursive lookups.
 */
...
BEGIN
    ...
    IF NOT has_generic OR NOT has_rows OR
       ...
       prefix_builds > 40 OR prefix_reuses < 500 THEN
        RAISE EXCEPTION
            'Generic Join prefix range directory reuse was not observed (builds %, reuses %)',
            prefix_builds, prefix_reuses;
    END IF;
    RAISE NOTICE 'generic join prefix range directory reuse verified';
END
