...
typedef enum AgeGenericPathProviderDescField
{
    ...
    AGE_GENERIC_PATH_PROVIDER_PHYSICAL_KIND,
    AGE_GENERIC_PATH_PROVIDER_ADJ_INDEX_OID,
...
static List *make_generic_ghd_separator_desc(
    int id, int parent_bag_id, int child_bag_id,
    AgeGenericGHDSeparatorKind kind, List *separator_variables,
    int separator_variable, int tail_variable, int provider_index,
    bool separator_is_key1);
static int generic_ghd_separator_kind_count(
    List *ghd_separators, AgeGenericGHDSeparatorKind kind);
...
static void mark_generic_edge_physical_path_desc(
    PlannerInfo *root, Path *provider_path, List *variable_rtis, List *desc);
static void mark_generic_lazy_physical_provider_descs(
    PlannerInfo *root, List *variable_rtis, List *provider_paths,
    List *provider_descs);
static bool generic_edge_provider_target_is_key_only(Path *provider_path,
                                                     List *desc);
...
static CustomPath *find_wcoj_path_requiring_outer_subset(
    RelOptInfo *rel, Relids required_outer_subset);
...
static Path *prepare_wcoj_adjacency_provider_path(
    ...
    List *output_exprs);
static Path *prepare_wcoj_source_dimension_provider_path(
    PlannerInfo *root, RelOptInfo *source_rel, List *provider_desc,
    List *output_exprs);
...
static bool add_wcoj_source_dimension_join_path(
...
static void add_wcoj_fractional_row_goal_paths(PlannerInfo *root,
                                               ...
                                               RelOptInfo *output_rel,
                                               FinalPathExtraData *extra);
static void add_generic_limit_row_goal_paths(PlannerInfo *root,
                                             RelOptInfo *input_rel,
                                             RelOptInfo *output_rel,
                                             FinalPathExtraData *extra);
static void add_generic_fractional_row_goal_paths(PlannerInfo *root,
                                                  RelOptInfo *input_rel,
                                                  RelOptInfo *output_rel,
                                                  FinalPathExtraData *extra);
...
static CustomPath *copy_wcoj_path_with_row_goal(CustomPath *wcoj_path,
                                                ...
                                                int consumer_kind,
                                                int row_goal_source);
static Path *make_generic_limit_row_goal_path(PlannerInfo *root,
                                              RelOptInfo *output_rel,
                                              LimitPath *limit_path);
static Path *make_generic_final_limit_row_goal_path(
    PlannerInfo *root, RelOptInfo *output_rel, Path *path,
    FinalPathExtraData *extra);
static Path *make_generic_fractional_row_goal_path(PlannerInfo *root,
                                                   RelOptInfo *output_rel,
                                                   Path *path,
                                                   int64 row_goal);
static CustomPath *copy_generic_path_with_row_goal(CustomPath *generic_path,
                                                   int64 row_goal,
                                                   int consumer_kind,
                                                   int row_goal_source);
...
static bool is_group_count_key_target(PathTarget *target,
                                      ...
                                      CustomPath *wcoj_path,
                                      Oid *output_type);
static bool is_group_count_distinct_key_target(PathTarget *target,
                                               AggPath *agg_path,
                                               CustomPath *wcoj_path,
                                               Oid *output_type);
...
static bool generic_count_clause_is_key_equality(CustomPath *generic_path,
                                                 Node *clause);
static bool is_count_distinct_generic_key_aggref(Aggref *aggref,
                                                 CustomPath *generic_path,
                                                 int *variable);
static int generic_distinct_arg_variable(CustomPath *generic_path, Node *arg);
...
static bool wcoj_sum_property_prepare_direct(
    ...
    List **paths, List **provider_descs, int *provider_index);
static bool add_wcoj_sum_property_local_qual_edge_exprs(
    List **output_exprs, List *local_quals, Index edge_rti);
...
static CustomPath *
find_wcoj_path_requiring_outer_subset(RelOptInfo *rel,
                                      Relids required_outer_subset)
{
    CustomPath *best = NULL;
    ListCell *lc;

    if (rel == NULL || bms_is_empty(required_outer_subset))
        return NULL;
    foreach(lc, rel->pathlist)
    {
        Path *path = (Path *)lfirst(lc);
        ...
        if (bms_is_empty(required_outer) ||
            !bms_is_subset(required_outer, required_outer_subset))
        {
            continue;
        }
        if (!IsA(path, CustomPath))
...
static bool
wcoj_source_key_expr_matches_rti(Node *source_key_expr, Index source_rti)
{
    Node *stripped;
    Var *var;

    if (source_rti <= 0 || source_key_expr == NULL)
        return false;
    stripped = strip_implicit_coercions(source_key_expr);
    if (!IsA(stripped, Var))
        return false;
    var = castNode(Var, stripped);
    return var->vartype == GRAPHIDOID &&
        var->varlevelsup == 0 &&
        var->varno == source_rti &&
        var->varattno == Anum_ag_label_vertex_table_id;
}

static List *
wcoj_source_dimension_output_exprs(PlannerInfo *root, RelOptInfo *joinrel,
                                   Path *source_path, List *restrictinfos,
                                   List *provider_desc)
{
    PathTarget *target;
    List *output_exprs;
    ...
    if (joinrel == NULL || source_path == NULL ||
        list_length(provider_desc) != AGE_WCOJ_PATH_PROVIDER_COUNT)
    {
        return NIL;
    }
    ...
    foreach(lc, output_exprs)
    {
        ...
        if (!pathtarget_contains_expr(target, expr))
        {
            add_column_to_pathtarget(target, (Expr *)copyObject(expr), 0);
        }
    }

    add_child_rel_vars_to_pathtarget(target, source_path,
                                     (Node *)joinrel->reltarget->exprs);
    add_child_join_clause_vars_to_pathtarget(target, source_path,
                                             restrictinfos);
    source_key_expr = list_nth(provider_desc,
                               AGE_WCOJ_PATH_PROVIDER_SOURCE_KEY_EXPR);
    if (!pathtarget_contains_expr(target, source_key_expr))
    {
        add_column_to_pathtarget(target,
                                 (Expr *)copyObject(source_key_expr), 0);
    }
    target = set_pathtarget_cost_width(root, target);
    return target->exprs;
}
...
static Path *
prepare_wcoj_source_dimension_provider_path(PlannerInfo *root,
                                            RelOptInfo *source_rel,
                                            List *provider_desc,
                                            List *output_exprs)
{
    ...
    source_key_expr = list_nth(provider_desc,
                               AGE_WCOJ_PATH_PROVIDER_SOURCE_KEY_EXPR);
    if (edge_rti <= 0 ||
        !wcoj_source_key_expr_matches_rti(
            source_key_expr,
            intVal(list_nth(provider_desc,
                            AGE_WCOJ_PATH_PROVIDER_SOURCE_RTI))) ||
        intVal(list_nth(provider_desc,
    ...
    foreach(lc, output_exprs)
    {
        ...
        if (!bms_is_subset(expr_relids, source_rel->relids))
        {
            bms_free(expr_relids);
            return NULL;
        }
    ...
    return copy_path_with_deferred_projection_target(root, source_path,
                                                     target, false);
}
                                ...
                                int provider_index, bool separator_is_key1)
{
    ...
    desc = lappend(desc, makeInteger(kind));
    desc = lappend(desc, separator_variables);
    return desc;
}

static int
generic_ghd_separator_kind_count(List *ghd_separators,
                                 AgeGenericGHDSeparatorKind kind)
{
    ListCell *lc;
    int count = 0;
...
static bool
try_make_generic_four_cycle_ghd(const AgeGenericHypergraph *hypergraph,
                                const bool *core_variables, List **ghd_bags,
                                List **ghd_separators, int *next_bag_id,
                                int *next_separator_id,
                                int *child_only_variable)
{
    int core_variable_count = 0;
    int core_edge_count = 0;
    int core_variables_list[4];
    int core_edge_indexes[4];
    ...
    for (index = 0; index < hypergraph->variable_count; index++)
    {
        if (!core_variables[index])
            continue;
        if (core_variable_count >= 4)
            return false;
        core_variables_list[core_variable_count++] = index;
    }
    if (core_variable_count != 4)
        return false;
    ...
    for (index = 0; index < hypergraph->edge_count; index++)
    {
        ...
        if (!core_variables[edge->var1] || !core_variables[edge->var2])
            continue;
        if (core_edge_count >= 4)
            return false;
        core_edge_indexes[core_edge_count++] = index;
        for (pos = 0; pos < core_variable_count; pos++)
        {
            if (core_variables_list[pos] == edge->var1)
                left_pos = pos;
            if (core_variables_list[pos] == edge->var2)
                right_pos = pos;
        }
    ...
    if (core_edge_count != 4)
        return false;
    for (index = 0; index < core_variable_count; index++)
    {
        if (degree[index] != 2)
            return false;
    }

    first_edge = core_edge_indexes[0];
    u = hypergraph->edges[first_edge].var1;
    v = hypergraph->edges[first_edge].var2;
    for (index = 1; index < core_edge_count; index++)
    {
        int edge_index = core_edge_indexes[index];
        const AgeGenericHypergraphEdge *edge = &hypergraph->edges[edge_index];

        if (edge->var1 == v && edge->var2 != u)
        {
            second_edge = edge_index;
            w = edge->var2;
            break;
        }
        if (edge->var2 == v && edge->var1 != u)
        {
            second_edge = edge_index;
            w = edge->var1;
            break;
    ...
    for (index = 1; index < core_edge_count; index++)
    {
        ...
        if (edge_index == second_edge)
            continue;
        if (edge->var1 == w && edge->var2 != v)
        {
            third_edge = edge_index;
            x = edge->var2;
            break;
        }
        if (edge->var2 == w && edge->var1 != v)
        {
            third_edge = edge_index;
            x = edge->var1;
            break;
    ...
    if (third_edge < 0 || x < 0 || x == u || x == v || x == w)
        return false;
    ...
    for (index = 1; index < core_edge_count; index++)
    {
        ...
        if (edge_index == second_edge || edge_index == third_edge)
            continue;
        if ((edge->var1 == x && edge->var2 == u) ||
            (edge->var2 == x && edge->var1 == u))
        {
            fourth_edge = edge_index;
            break;
        }
    }
    if (fourth_edge < 0)
    ...
    parent_bag_id = (*next_bag_id)++;
    child_bag_id = (*next_bag_id)++;
    *ghd_bags = lappend(
        *ghd_bags,
        make_generic_ghd_bag_desc(
            parent_bag_id, AGE_GRAPH_JOIN_GHD_BAG_CYCLIC_CORE,
            list_make3(makeInteger(u), makeInteger(v), makeInteger(w)),
            list_make2(makeInteger(hypergraph->variable_count + first_edge),
                       makeInteger(hypergraph->variable_count +
                                   second_edge))));
    *ghd_bags = lappend(
        *ghd_bags,
        make_generic_ghd_bag_desc(
            child_bag_id, AGE_GRAPH_JOIN_GHD_BAG_CYCLIC_CORE,
            list_make3(makeInteger(u), makeInteger(x), makeInteger(w)),
            list_make2(makeInteger(hypergraph->variable_count + fourth_edge),
                       makeInteger(hypergraph->variable_count +
                                   third_edge))));
    *ghd_separators = lappend(
        *ghd_separators,
        make_generic_ghd_separator_desc(
            (*next_separator_id)++, parent_bag_id, child_bag_id,
            AGE_GENERIC_GHD_SEPARATOR_PAIR,
            list_make2(makeInteger(u), makeInteger(w)),
            u, w, hypergraph->variable_count + fourth_edge, true));
    *child_only_variable = x;
    return true;
}
                          ...
                          List **ghd_separators)
{
    ...
    int leaf_bag_id = 0;
    int separator_id = 0;
    int split_child_only_variable = -1;
    bool split_core = false;
    int index;
    ...
    *ghd_separators = NIL;
    ...
    split_core = try_make_generic_four_cycle_ghd(
        hypergraph, core_variables, ghd_bags, ghd_separators, &leaf_bag_id,
        &separator_id, &split_child_only_variable);
    if (!split_core)
    {
        *ghd_bags = lappend(
            *ghd_bags,
    ...
    for (index = 0; index < hypergraph->edge_count; index++)
    {
        ...
        parent_bag_id = split_core &&
            separator_variable == split_child_only_variable ? 1 : 0;
        *ghd_bags = lappend(
            *ghd_bags,
            make_generic_ghd_bag_desc(leaf_bag_id,
        ...
        *ghd_separators = lappend(
            *ghd_separators,
            make_generic_ghd_separator_desc(
                separator_id, parent_bag_id, leaf_bag_id,
                AGE_GENERIC_GHD_SEPARATOR_LEAF_TAIL,
                list_make1(makeInteger(separator_variable)),
                separator_variable,
...
static bool
generic_reduction_has_ghd_separators(List *reduction_desc)
{
    if (list_length(reduction_desc) != AGE_GENERIC_REDUCTION_DESC_COUNT)
        return false;

    return intVal(list_nth(
        reduction_desc, AGE_GENERIC_REDUCTION_DESC_GHD_SEPARATOR_COUNT)) > 0;
}
...
make_generic_vertex_path_desc(Index rti, int variable, Expr *key_expr)
{
    ...
    desc = lappend(desc, make_wcoj_oid_const(InvalidOid));
    desc = lappend(desc, makeInteger(0));
...
static void
mark_generic_edge_physical_path_desc(PlannerInfo *root, Path *provider_path,
                                     List *variable_rtis, List *desc)
{
    CypherWCOJTerminalEntry *entry;
    RelOptInfo *rel;
    Index edge_rti;
    ...
    if (root == NULL || provider_path == NULL || variable_rtis == NIL ||
        desc == NIL ||
        list_length(desc) != AGE_GENERIC_PATH_PROVIDER_COUNT)
    {
        return;
    }
    if (intVal(list_nth(desc, AGE_GENERIC_PATH_PROVIDER_KIND)) !=
        AGE_GENERIC_PROVIDER_EDGE)
    {
        return;
    }

    edge_rti = (Index)intVal(list_nth(
        desc, AGE_GENERIC_PATH_PROVIDER_REL_RTI));
    entry = find_wcoj_terminal_entry(root, edge_rti);
    if (entry == NULL)
        return;
    var1 = intVal(list_nth(desc, AGE_GENERIC_PATH_PROVIDER_VAR1));
    var2 = intVal(list_nth(desc, AGE_GENERIC_PATH_PROVIDER_VAR2));
    source_variable = generic_variable_position(variable_rtis,
                                                entry->source_rti);
    terminal_variable = generic_variable_position(variable_rtis,
                                                  entry->terminal_rti);

    if (!OidIsValid(entry->index_oid) ||
        entry->exact_terminal_bound ||
        source_variable != var1 ||
        terminal_variable != var2 ||
        entry->edge_rti <= 0 ||
        entry->edge_rti >= root->simple_rel_array_size)
    {
        return;
    ...
    rel = root->simple_rel_array[entry->edge_rti];
    if (rel == NULL || rel->baserestrictinfo != NIL ||
        !generic_edge_provider_target_is_key_only(provider_path, desc))
    {
        return;
    }

    lfirst(list_nth_cell(desc, AGE_GENERIC_PATH_PROVIDER_PHYSICAL_KIND)) =
        makeInteger(AGE_GENERIC_PROVIDER_PHYSICAL_ADJACENCY_EDGE);
    lfirst(list_nth_cell(desc, AGE_GENERIC_PATH_PROVIDER_ADJ_INDEX_OID)) =
        make_wcoj_oid_const(entry->index_oid);
...
static void
mark_generic_lazy_physical_provider_descs(PlannerInfo *root,
                                          List *variable_rtis,
                                          List *provider_paths,
                                          List *provider_descs)
{
    ...
    if (root == NULL || variable_rtis == NIL ||
        list_length(provider_paths) != list_length(provider_descs))
    {
        return;
    }

    forboth(path_cell, provider_paths, desc_cell, provider_descs)
    {
        Path *provider_path = lfirst(path_cell);
        List *desc = lfirst(desc_cell);

        mark_generic_edge_physical_path_desc(root, provider_path,
                                             variable_rtis, desc);
    }
}

static bool
generic_edge_provider_target_is_key_only(Path *provider_path, List *desc)
{
    ListCell *lc;
    Node *key1_expr;
    Node *key2_expr;
    ...
    if (provider_path == NULL || provider_path->pathtarget == NULL ||
        desc == NIL || list_length(desc) != AGE_GENERIC_PATH_PROVIDER_COUNT)
    {
        return false;
    }
    output_count = list_length(provider_path->pathtarget->exprs);
    if (output_count <= 0 || output_count > 3)
        return false;

    key1_expr = strip_implicit_coercions(
        list_nth(desc, AGE_GENERIC_PATH_PROVIDER_KEY1_EXPR));
    key2_expr = strip_implicit_coercions(
        list_nth(desc, AGE_GENERIC_PATH_PROVIDER_KEY2_EXPR));
    edge_id_expr = strip_implicit_coercions(
        list_nth(desc, AGE_GENERIC_PATH_PROVIDER_EDGE_ID_EXPR));

    foreach(lc, provider_path->pathtarget->exprs)
    {
        Node *expr = strip_implicit_coercions(lfirst(lc));

        if (!equal(expr, key1_expr) &&
            !equal(expr, key2_expr) &&
            !equal(expr, edge_id_expr))
        {
            return false;
        }
    }
                      ...
                      JoinType jointype, JoinPathExtraData *extra)
{
    ...
    if (hypergraph == NULL || hypergraph->component_count <= 0)
        return;
    if (hypergraph->component_count != 1 && hypergraph->cyclic)
        return;
...
static bool
add_wcoj_source_dimension_join_path(PlannerInfo *root, RelOptInfo *joinrel,
                                    RelOptInfo *outerrel,
                                    ...
                                    JoinPathExtraData *extra)
{
    ...
    if (bms_membership(outerrel->relids) == BMS_SINGLETON)
    {
        source_rel = outerrel;
        wcoj_rel = innerrel;
    }
    else if (bms_membership(innerrel->relids) == BMS_SINGLETON)
    {
        source_rel = innerrel;
        wcoj_rel = outerrel;
    }
    ...
    wcoj_path = find_wcoj_path_requiring_outer_subset(wcoj_rel,
                                                      source_rel->relids);
    if (wcoj_path == NULL ||
    ...
    stored_provider_descs = list_nth(
        wcoj_path->custom_private,
        AGE_WCOJ_PATH_PRIVATE_PROVIDER_DESCS);
    if (list_length(stored_keys) != list_length(wcoj_path->custom_paths) ||
        list_length(stored_provider_descs) !=
        list_length(wcoj_path->custom_paths))
    {
        return false;
             ...
             desc_cell, stored_provider_descs)
    {
        ...
        source_rti = intVal(list_nth(provider_desc,
                                     AGE_WCOJ_PATH_PROVIDER_SOURCE_RTI));
        source_key_expr = list_nth(
            provider_desc, AGE_WCOJ_PATH_PROVIDER_SOURCE_KEY_EXPR);
        if (!bms_is_member(source_rti, source_rel->relids) ||
            !wcoj_source_key_expr_matches_rti(source_key_expr, source_rti))
        {
            return false;
        }

        output_exprs = wcoj_source_dimension_output_exprs(
            root, joinrel, source_path, restrictinfos, provider_desc);
        if (output_exprs == NIL)
            return false;
        lfirst(list_nth_cell(
            provider_desc, AGE_WCOJ_PATH_PROVIDER_OUTPUT_EXPRS)) =
        ...
        local_quals = list_nth(provider_desc,
                               AGE_WCOJ_PATH_PROVIDER_LOCAL_QUALS);
        payload_mask = wcoj_edge_payload_mask(
            output_exprs, local_quals,
            intVal(list_nth(provider_desc,
                            AGE_WCOJ_PATH_PROVIDER_EDGE_RTI)));
        lfirst(list_nth_cell(
            provider_desc, AGE_WCOJ_PATH_PROVIDER_PAYLOAD_MASK)) =
        ...
        prepared_path = prepare_wcoj_source_dimension_provider_path(
            root, source_rel, provider_desc, output_exprs);
        if (prepared_path == NULL)
            return false;

        posting_estimate = wcoj_provider_posting_estimate(
            root, component_path, provider_desc);
        build.paths = lappend(build.paths, prepared_path);
        key_exprs = lappend(key_exprs, copyObject(key_expr));
        build.provider_descs = lappend(build.provider_descs, provider_desc);
        build.estimated_postings = lappend(
    ...
    if (adjacency_provider_count < 2 ||
        adjacency_provider_count != list_length(build.paths))
    {
        return false;
    }
                   ...
                   JoinType jointype, JoinPathExtraData *extra)
{
    ...
    if (!age_enable_wcoj)
        return;
    if (add_wcoj_source_dimension_join_path(root, joinrel, outerrel,
                               ...
                               RelOptInfo *output_rel, void *extra)
{
    ...
    add_wcoj_fractional_row_goal_paths(root, input_rel, output_rel,
                                       (FinalPathExtraData *)extra);
    add_generic_limit_row_goal_paths(root, input_rel, output_rel,
                                     (FinalPathExtraData *)extra);
    add_generic_fractional_row_goal_paths(root, input_rel, output_rel,
                                          (FinalPathExtraData *)extra);
...
static void
add_wcoj_distinct_key_paths(PlannerInfo *root, RelOptInfo *output_rel)
{
    List *new_paths = NIL;
    ListCell *lc;
                             ...
                             int consumer_kind, int row_goal_source)
{
    ...
    existing_consumer_kind = intVal(list_nth(
        wcoj_path->custom_private, AGE_WCOJ_PATH_PRIVATE_CONSUMER));
    if (existing_consumer_kind != AGE_WCOJ_CONSUMER_ROWS &&
        existing_consumer_kind != AGE_WCOJ_CONSUMER_DISTINCT_KEY)
    {
        return NULL;
    }
    if (((existing_consumer_kind == AGE_WCOJ_CONSUMER_ROWS &&
    ...
    row_goal_path->path.rows = consumer_kind == AGE_WCOJ_CONSUMER_EXISTS ?
...
static void
add_generic_limit_row_goal_paths(PlannerInfo *root, RelOptInfo *input_rel,
                                 RelOptInfo *output_rel,
                                 FinalPathExtraData *extra)
{
    List *new_paths = NIL;
    ...
    foreach(lc, output_rel->pathlist)
    {
        ...
        if (IsA(path, LimitPath))
            row_goal_path = make_generic_limit_row_goal_path(
                root, output_rel, (LimitPath *)path);
        else
            row_goal_path = make_generic_final_limit_row_goal_path(
                root, output_rel, path, extra);
        if (row_goal_path != NULL)
            new_paths = lappend(new_paths, row_goal_path);
    }
...
static void
add_generic_fractional_row_goal_paths(PlannerInfo *root,
                                      RelOptInfo *input_rel,
                                      RelOptInfo *output_rel,
                                      FinalPathExtraData *extra)
{
    ...
    foreach(lc, output_rel->pathlist)
    {
        ...
        row_goal_path = make_generic_fractional_row_goal_path(
            root, output_rel, path, row_goal);
        if (row_goal_path != NULL)
            new_paths = lappend(new_paths, row_goal_path);
    }
...
static Path *
make_generic_final_limit_row_goal_path(PlannerInfo *root,
                                       RelOptInfo *output_rel, Path *path,
                                       FinalPathExtraData *extra)
{
    CustomPath *generic_path;
    CustomPath *row_goal_subpath;
    ...
    generic_path = (CustomPath *)path;
    if (generic_path->methods != &age_generic_join_path_methods)
        return NULL;

    row_goal_subpath = copy_generic_path_with_row_goal(
        generic_path, extra->count_est, AGE_GENERIC_CONSUMER_LIMIT,
    ...
    ereport(DEBUG2,
            (errmsg_internal("AGE Generic Join final limit row-goal path "
                             "added: row_goal=" INT64_FORMAT
                             " original_cost=%.2f new_cost=%.2f",
                             extra->count_est, path->total_cost,
                             row_goal_path->total_cost)));
...
static Path *
make_generic_fractional_row_goal_path(PlannerInfo *root,
                                      RelOptInfo *output_rel, Path *path,
                                      int64 row_goal)
{
    CustomPath *generic_path;
    ...
    generic_path = (CustomPath *)path;
    if (generic_path->methods != &age_generic_join_path_methods ||
        row_goal != 1 || projection_target != NULL ||
        generic_path->path.pathtarget == NULL ||
        generic_path->path.pathtarget->exprs != NIL ||
        !generic_count_restrictinfos_supported(generic_path))
    {
        return NULL;
    }

    row_goal_path = copy_generic_path_with_row_goal(
        generic_path, row_goal, AGE_GENERIC_CONSUMER_EXISTS,
    ...
    ereport(DEBUG2,
            (errmsg_internal("AGE Generic Join exists row-goal path added: "
                             "row_goal=" INT64_FORMAT
...
static Path *
make_generic_limit_row_goal_path(PlannerInfo *root, RelOptInfo *output_rel,
                                 LimitPath *limit_path)
{
    CustomPath *generic_path;
    CustomPath *row_goal_subpath;
    ...
    generic_path = (CustomPath *)limit_path->subpath;
    if (generic_path->methods != &age_generic_join_path_methods)
        return NULL;

    row_goal_subpath = copy_generic_path_with_row_goal(
        generic_path, row_goal, AGE_GENERIC_CONSUMER_LIMIT,
        AGE_GENERIC_ROW_GOAL_LIMIT);
    if (row_goal_subpath == NULL)
        return NULL;
    ...
    ereport(DEBUG2,
            (errmsg_internal("AGE Generic Join limit row-goal path added: "
                             "row_goal=" INT64_FORMAT
                             " original_cost=%.2f new_cost=%.2f",
                             row_goal,
                             limit_path->path.total_cost,
...
static CustomPath *
copy_generic_path_with_row_goal(CustomPath *generic_path, int64 row_goal,
                                int consumer_kind, int row_goal_source)
{
    CustomPath *row_goal_path;
    Const *existing_row_goal;
    ...
    if (generic_path == NULL || row_goal <= 0 ||
    ...
    existing_consumer_kind = intVal(list_nth(
        generic_path->custom_private, AGE_GENERIC_PATH_PRIVATE_CONSUMER));
    if (existing_consumer_kind != AGE_GENERIC_CONSUMER_ROWS)
        return NULL;
    if (consumer_kind != AGE_GENERIC_CONSUMER_LIMIT &&
    ...
    existing_row_goal = list_nth_node(
        Const, generic_path->custom_private,
        AGE_GENERIC_PATH_PRIVATE_ROW_GOAL);
    if (existing_row_goal->constisnull ||
        existing_row_goal->consttype != INT8OID)
    {
    ...
    row_goal_path = makeNode(CustomPath);
    memcpy(row_goal_path, generic_path, sizeof(*row_goal_path));
    row_goal_path->path.rows = consumer_kind == AGE_GENERIC_CONSUMER_EXISTS ?
        1.0 : clamp_row_est(Min(generic_path->path.rows, (double)row_goal));
    row_fraction = (double)row_goal / Max(generic_path->path.rows, 1.0);
    if (row_fraction < 1.0)
    {
        Cost run_cost = Max(generic_path->path.total_cost -
                            generic_path->path.startup_cost, 0.0);
    ...
    row_goal_path->custom_paths = list_copy(generic_path->custom_paths);
    row_goal_path->custom_restrictinfo = generic_path->custom_restrictinfo;
    row_goal_path->custom_private = list_make4(
    ...
    row_goal_path->custom_private = lappend(
        row_goal_path->custom_private,
        consumer_kind == AGE_GENERIC_CONSUMER_EXISTS ?
        make_wcoj_oid_const(AGTYPEOID) : make_wcoj_oid_const(InvalidOid));
    row_goal_path->custom_private = lappend(row_goal_path->custom_private,
                                            make_int8_const(row_goal));
    row_goal_path->custom_private = lappend(row_goal_path->custom_private,
...
static bool
is_group_count_distinct_key_target(PathTarget *target, AggPath *agg_path,
                                   CustomPath *wcoj_path, Oid *output_type)
{
    SortGroupClause *group_clause;
    Node *group_expr;
...
static bool
is_wcoj_distinct_key_target(PathTarget *target, CustomPath *wcoj_path,
                            Oid *output_type)
{
    Node *expr;
    Oid expr_type;
    ...
    if (!wcoj_distinct_arg_matches_key(wcoj_path, expr))
        return false;
...
static CustomPath *
make_wcoj_distinct_key_path(PlannerInfo *root, RelOptInfo *output_rel,
                            Path *distinct_path)
{
    ...
    if (IsA(distinct_path, UpperUniquePath))
    {
        ...
        wcoj_input_path = unique_path->subpath;
    }
    ...
    for (;;)
    {
        ...
        if (IsA(wcoj_input_path, ProjectionPath))
        {
            ProjectionPath *projection_path =
                (ProjectionPath *)wcoj_input_path;
    ...
    if (wcoj_path->methods != &age_wcoj_join_path_methods ||
        !wcoj_count_path_is_direct(wcoj_path) ||
        !wcoj_count_restrictinfos_supported(wcoj_path) ||
        !is_wcoj_distinct_key_target(target, wcoj_path, &output_type))
    {
        return NULL;
    }
    ...
    key_path->path.disabled_nodes = wcoj_path->path.disabled_nodes;
    key_path->path.startup_cost = wcoj_path->path.startup_cost;
    key_path->path.total_cost = wcoj_path->path.total_cost +
    ...
    key_path->custom_paths = list_copy(wcoj_path->custom_paths);
    key_path->custom_restrictinfo = wcoj_path->custom_restrictinfo;
    key_path->custom_private = list_make3(
        copyObject(key_exprs), copyObject(estimated_postings),
        copyObject(provider_descs));
    key_path->custom_private = lappend(
        key_path->custom_private,
        makeInteger(AGE_WCOJ_CONSUMER_DISTINCT_KEY));
    key_path->custom_private = lappend(
        key_path->custom_private, make_wcoj_oid_const(output_type));
    key_path->custom_private = lappend(
        key_path->custom_private, make_int8_const(0));
    key_path->custom_private = lappend(
        key_path->custom_private, makeInteger(AGE_WCOJ_ROW_GOAL_NONE));
    key_path->custom_private = lappend(key_path->custom_private, NIL);
    key_path->methods = &age_wcoj_join_path_methods;

    ereport(DEBUG2,
            (errmsg_internal("AGE WCOJ distinct-key path added: sources=%d "
                             "source_cost=%.2f distinct_cost=%.2f "
                             "output_type=%u",
                             list_length(wcoj_path->custom_paths),
                             wcoj_path->path.total_cost,
                     ...
                     AggPath *agg_path)
{
    ...
    if (agg_path->groupClause != NIL)
    {
        ...
        else if (is_group_count_distinct_key_target(
                     agg_path->path.pathtarget, agg_path, wcoj_path,
                     &output_type))
        {
                        ...
                        AggPath *agg_path)
{
    ...
    int consumer_kind;
    int distinct_variable = -1;
    List *count_variable_rtis;
    List *count_provider_descs;
    List *count_reduction_desc;
    ...
    count_provider_descs = list_nth(
        count_path->custom_private, AGE_GENERIC_PATH_PRIVATE_PROVIDER_DESCS);
    count_reduction_desc = list_nth(
        count_path->custom_private, AGE_GENERIC_PATH_PRIVATE_REDUCTION_DESC);
    if (!generic_reduction_has_ghd_separators(count_reduction_desc))
    {
        mark_generic_lazy_physical_provider_descs(root, count_variable_rtis,
                                                  count_path->custom_paths,
                                                  count_provider_descs);
    }
...
static bool
add_wcoj_sum_property_local_qual_edge_exprs(List **output_exprs,
                                            List *local_quals,
                                            Index edge_rti)
{
    List *vars;
    ...
    if (output_exprs == NULL || edge_rti == 0)
        return false;

    vars = pull_var_clause((Node *)local_quals,
    ...
    foreach(lc, vars)
    {
        ...
        if (var->varno != edge_rti || var->varlevelsup != 0)
            continue;
        if (!wcoj_direct_edge_expr((Node *)var, edge_rti))
        {
            list_free(vars);
            return false;
        }
        if (!list_member(*output_exprs, var))
            *output_exprs = lappend(*output_exprs, copyObject(var));
    }
    ...
    *provider_descs = copyObject(list_nth(
    ...
    foreach(desc_cell, *provider_descs)
    {
        ...
        if (provider_edge_rti == edge_rti)
        {
            ...
            local_quals = list_nth(provider_desc,
                                   AGE_WCOJ_PATH_PROVIDER_LOCAL_QUALS);
            if (!add_wcoj_sum_property_local_qual_edge_exprs(
                    &output_exprs, local_quals, edge_rti))
            {
                return false;
            }
            payload_mask = wcoj_edge_payload_mask(output_exprs, local_quals,
                                                  edge_rti);
            lfirst(list_nth_cell(
                provider_desc, AGE_WCOJ_PATH_PROVIDER_KIND)) =
                makeInteger(AGE_WCOJ_PROVIDER_ADJACENCY);
...
static bool
is_count_distinct_generic_key_aggref(Aggref *aggref, CustomPath *generic_path,
                                     int *variable)
{
    TargetEntry *arg_tle;
    int key_variable;
    ...
    key_variable = generic_distinct_arg_variable(
        generic_path, (Node *)arg_tle->expr);
    if (key_variable < 0)
        return false;
    if (variable != NULL)
...
static int
generic_distinct_arg_variable(CustomPath *generic_path, Node *arg)
{
    Node *key_arg;
    List *provider_descs;
    ListCell *lc;
    ...
    foreach(lc, provider_descs)
    {
        ...
        if (key1_expr != NULL && exprType(key1_expr) == GRAPHIDOID &&
            equal(key_arg, key1_expr))
        {
            return intVal(list_nth(provider_desc,
                                   AGE_GENERIC_PATH_PROVIDER_VAR1));
        }
        ...
        if (provider_kind == AGE_GENERIC_PROVIDER_EDGE)
        {
            key2_expr = strip_implicit_coercions(
                list_nth(provider_desc,
                         AGE_GENERIC_PATH_PROVIDER_KEY2_EXPR));
            if (key2_expr != NULL && exprType(key2_expr) == GRAPHIDOID &&
                equal(key_arg, key2_expr))
            {
                return intVal(list_nth(provider_desc,
                                       AGE_GENERIC_PATH_PROVIDER_VAR2));
            }
...
static bool
wcoj_provider_source_correlation_match(List *left_desc, List *right_desc)
{
    Index left_source_rti;
    Index right_source_rti;
    Node *left_source_key_expr;
    Node *right_source_key_expr;

    if (list_length(left_desc) != AGE_WCOJ_PATH_PROVIDER_COUNT ||
        list_length(right_desc) != AGE_WCOJ_PATH_PROVIDER_COUNT)
    {
        return false;
    }

    left_source_rti = intVal(list_nth(
        left_desc, AGE_WCOJ_PATH_PROVIDER_SOURCE_RTI));
    right_source_rti = intVal(list_nth(
        right_desc, AGE_WCOJ_PATH_PROVIDER_SOURCE_RTI));
    if (left_source_rti <= 0 || right_source_rti <= 0 ||
        left_source_rti != right_source_rti)
    {
        return false;
    }

    left_source_key_expr = list_nth(
        left_desc, AGE_WCOJ_PATH_PROVIDER_SOURCE_KEY_EXPR);
    right_source_key_expr = list_nth(
        right_desc, AGE_WCOJ_PATH_PROVIDER_SOURCE_KEY_EXPR);
    if (left_source_key_expr == NULL || right_source_key_expr == NULL ||
        (IsA(left_source_key_expr, Const) &&
         castNode(Const, left_source_key_expr)->constisnull) ||
        (IsA(right_source_key_expr, Const) &&
         castNode(Const, right_source_key_expr)->constisnull))
    {
        return false;
    }

    return equal(strip_implicit_coercions(left_source_key_expr),
                 strip_implicit_coercions(right_source_key_expr));
}

static int *
build_wcoj_source_correlation_groups(List *provider_descs)
{
    int provider_count = list_length(provider_descs);
    int *groups;
    int next_group = 1;
    ListCell *outer_cell;
    int outer_index = 0;

    groups = palloc0(sizeof(int) * provider_count);
    foreach(outer_cell, provider_descs)
    {
        List *outer_desc = (List *)lfirst(outer_cell);
        ListCell *inner_cell;
        bool matched = false;
        int inner_index = 0;

        if (groups[outer_index] != 0)
        {
            outer_index++;
            continue;
        }

        foreach(inner_cell, provider_descs)
        {
            List *inner_desc = (List *)lfirst(inner_cell);

            if (inner_index <= outer_index || groups[inner_index] != 0)
            {
                inner_index++;
                continue;
            }
            if (wcoj_provider_source_correlation_match(outer_desc,
                                                       inner_desc))
            {
                if (!matched)
                {
                    groups[outer_index] = next_group;
                    matched = true;
                }
                groups[inner_index] = next_group;
            }
            inner_index++;
        }
        if (matched)
                              ...
                              int source_correlation_group)
{
    ...
    desc = lappend(desc, copyObject(list_nth(
        path_desc, AGE_WCOJ_PATH_PROVIDER_SOURCE_RTI)));
    desc = lappend(desc, makeInteger(source_correlation_group));
                             ...
                             List **provider_local_quals)
{
    ...
    *provider_local_quals = NIL;
    ...
    source_correlation_groups =
        build_wcoj_source_correlation_groups(provider_descs);
...
static List *
build_wcoj_distinct_key_custom_scan_tlist(PathTarget *target,
                                          List *provider_tlist)
{
    List *key_tlist;
    TargetEntry *key_tle;
    ...
    if (target == NULL || list_length(target->exprs) != 1)
        elog(ERROR, "invalid AGE WCOJ distinct-key target");

    key_tle = makeTargetEntry((Expr *)copyObject(linitial(target->exprs)),
                              1, pstrdup("wcoj_distinct_key"), false);
    if (target->sortgrouprefs != NULL)
...
static List *
build_generic_exists_custom_scan_tlist(List *provider_tlist)
{
    Const *exists_value;
    TargetEntry *exists_tle;
                           ...
                           List *clauses, List *custom_plans)
{
    ...
    if (row_goal < 0 ||
        (consumer_kind != AGE_GENERIC_CONSUMER_LIMIT &&
         consumer_kind != AGE_GENERIC_CONSUMER_EXISTS && row_goal != 0) ||
        (consumer_kind == AGE_GENERIC_CONSUMER_LIMIT &&
    ...
    {
        custom_scan_tlist = build_generic_exists_custom_scan_tlist(
            custom_scan_tlist);
    }
                        ...
                        List *clauses, List *custom_plans)
{
    ...
    else if (consumer_kind == AGE_WCOJ_CONSUMER_DISTINCT_KEY)
    {
        custom_scan_tlist = build_wcoj_distinct_key_custom_scan_tlist(
            best_path->path.pathtarget, custom_scan_tlist);
    }
